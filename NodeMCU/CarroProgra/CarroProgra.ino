...
const char* ssid = "Joaquin";//Nombre de la red a la que se desea conectar
...
int data = 0; //Valor luces
bool LF = 0; //Luz frontal
bool LT = 0; //Luz trasera
bool LD = 0; //Luz derecha
bool LI = 0; //Luz izquiera
bool Sen = 0; //Guarda el sentido de las llantas, 1 = izquiera, 0 = derecha
bool Frt = 0; //Preguntar funcion 
...
void setup() {
  ...
  pinMode(ldr,INPUT); //Fotoresistencia 

  // ip estática para el servidor
  IPAddress ip(192,168,43,102); //ipv4 -1(103-102)
  IPAddress gateway(192,168,43,122);//puerta de enlace
...
String implementar(String llave, String valor){
   ...
   */
  String result="ok;"; // Importa apartir de acá
  ...
  if(llave == "pwm"){
    ...
    //Cambiar
    if (valor.toInt() >= -1023 && valor.toInt() < -500){
      centrarDireccion(); //Centra las llantas
      delay(200); //Mismas instrucciones que la función retroceder 
    ...
    if (valor.toInt() <= 1023 && valor.toInt() > 500){
      centrarDireccion(); //Centra las llantas
      delay(200); //Mismas instrucciones que la función avanzar
 ...
 //Modificar giro derecha
    ...
    switch (llave[1]){
      case 'f':
        ...
        //# AGREGAR CÓDIGO PARA ENCENDER O APAGAR LUCES FRONTALES
        //Modificar código
        switch (valor.toInt()){
          case 1:
            Serial.println("Encendiendo luces frontales");
            LFON();
            break;
          case 0:
            Serial.println("Apagando luces frontales");
            LFOFF();
            break;
        }
        break;
      case 'b':
        ...
        switch (valor.toInt()){
          case 1:
            Serial.println("Encendiendo luces traseras");
            LTON();
            break;
          case 0:
            Serial.println("Apagando luces traseras");
            LTOFF();
            break;
        }
        break;
      case 'l':
        ...
        switch (valor.toInt()){
          case 1:
            Serial.println("Encendiendo la direccional izquierda");
            LION();
            break;
          case 0:
            Serial.println("Apagando la direccional izquierda");
            LIOFF();
            break;
        }
        break; 
      case 'r':
        ...
        switch (valor.toInt()){
          case 1:
            Serial.println("Encendiendo la direccional derecha");
            LDON();
            break;
          case 0:
            Serial.println("Apagando la direccional derecha");
            LDOFF();
            break;
        }
        break;
...
// Función para que el carro se mueva a la derecha
void dirDer(){
  Sen=0; //Sen=0 indica que las llantas están a la derecha
  Frt=0;
...
// Función para que el carro se mueva a la izquierda
void dirIzq(){
  Sen=1; //Sen=1 indica que las llantas están a la izquierda
  Frt=0;
  digitalWrite(In3,LOW);
  digitalWrite(In4,LOW);
  delay(200);
...
// Función para que el carro avance 
void dirAde(){
  centrarDireccion(); //Centra las llantas
  delay(200);
  digitalWrite(In2,HIGH);
...
// Función para que el carro retroceda
void dirRet(){
  centrarDireccion(); //Centra las llantas
  delay(200);
  digitalWrite(In1,HIGH);
...
void centrarDireccion(){
  if(Sen=1){ //Llantas están a la izquierda
    digitalWrite(In3,LOW);
    digitalWrite(In4,LOW);
    delay(200);
    digitalWrite(In3,HIGH);
    ...
    if(Sen=0){ //Llantas están a la derecha
    digitalWrite(In3,LOW);
    digitalWrite(In4,LOW);
    delay(200);
    digitalWrite(In4,HIGH);
